...
# cd /home/chengrongxian/git/openmines
# tensorboard --logdir /home/chengrongxian/git/openmines/runs/mine --bind_all
...
class Args:
    ...
    update_epochs: int = 2  # 减少更新轮数，防止过拟合
    ...
    norm_adv: bool = True
    vf_coef: float = 0.5
    target_kl: float = 0.015 # 添加早停机制，防止过拟合
    ...
    guide_initial_value: float = 0.1     # 指导初始值：初始教师指导强度
    guide_final_value: float = 0.0       # 指导最终值：最终教师指导强度(衰减到0)
    guide_start_decay_step: int = 3_000_000  # 开始衰减步数：在第300万步开始衰减指导强度  感觉可能到1M就够了看曲线
    guide_decay_steps: int = 500_000     # 衰减持续步数：衰减过程持续50万步
    teacher_acceptable_tons: float = 12000    # 可接受产量阈值：当产量达到12000吨时停止教师指导
    teacher_alpha: float = 0 #0        # 教师指导系数：控制教师指导的权重
...
class TransformerActor(nn.Module):
    """Transformer-based Actor network for policy learning with optimized parameters"""
    
    def __init__(self, input_dim, hidden_dim, output_dim, num_heads=4, num_layers=1, dropout=0.15):
        super().__init__()
        ...
        # 输入投影层：使用更小的初始化标准差
        self.input_projection = layer_init(nn.Linear(input_dim, hidden_dim), std=0.1)
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=hidden_dim,
            nhead=num_heads,
            dim_feedforward=hidden_dim * 2,  # 减小前馈网络的维度
            dropout=dropout,
            batch_first=True,
        ...
        self.residual = nn.Sequential(
            layer_init(nn.Linear(input_dim, hidden_dim), std=0.1),
            nn.GELU(),
            nn.Dropout(dropout)
        )
        
        # 输出投影层使用更小的初始化标准差
        self.output_projection = layer_init(nn.Linear(hidden_dim, output_dim), std=0.005)
    ...
    def forward(self, x):
        ...
        # 主路径：通过Transformer
        main = self.input_projection(x)  # (batch_size, hidden_dim)
        main = main.unsqueeze(1)  # (batch_size, 1, hidden_dim)
        main = main + self.pos_encoding
        
        # 预归一化
        main = self.pre_norm(main)
        
        # Transformer处理
        main = self.transformer(main)  # (batch_size, 1, hidden_dim)
        main = main.squeeze(1)  # (batch_size, hidden_dim)
        
        # 后归一化
        main = self.post_norm(main)
        
        # 残差路径
        residual = self.residual(x)
        
        # 合并主路径和残差
        combined = main + residual
        
        # 输出投影
        logits = self.output_projection(combined)  # (batch_size, output_dim)
...
class Agent(nn.Module):
    def __init__(self, envs, args, norm_path=None):
        super().__init__()
        # 直接从配置文件读取装载点和卸载点数量，而不像此前那样固定写死5个装载点和5个卸载点
        import json
        with open(args.mine_config, 'r') as f:
            config = json.load(f)
        
        self.load_sites_num = len(config['load_sites'])
        self.dump_sites_num = len(config['dump_sites'])
...
class CheckpointManager:
    ...
    def save_checkpoint(self,
        ...
        # 在save_checkpoint方法中（第295-300行）
        if not self.args.save_best_only:
            checkpoint_path = os.path.join(self.checkpoint_dir, filename)
            torch.save(checkpoint, checkpoint_path)
        else:
            # 修复：即使是save_best_only模式，也应该在满足保存条件时保存
            checkpoint_path = os.path.join(self.checkpoint_dir, filename)
            if is_best:  # 只有在is_best时才保存
                torch.save(checkpoint, checkpoint_path)
...
if __name__ == "__main__":
    args = tyro.cli(Args)
    # compute batch sizes
    args.batch_size = args.num_envs * args.num_steps # 收集batch_size条经验,将其分成小批次用于模型训练
    ...
    # 创建环境，具体在make_env函数中;下面定义了调用该函数，更早还在rl_env里
    # 此时50个矿山仿真环境已经自动启动 ，每个环境都在独立的子进程中运行完整的矿山仿真系统。
    ...
    for iteration in range(start_iteration + 1, args.num_iterations + 1):
        ...
        if args.anneal_lr:
            frac = 1.0 - 0.5 * (iteration - 1.0) / args.num_iterations  # 只衰减到50%
            lrnow = max(frac * args.learning_rate, args.learning_rate * 0.1)  # 最小学习率为初始值的10%
        ...
        if current_normalized_reward > max_avg_tons:
            max_avg_tons = current_normalized_reward
            is_best = True

        if latest_produce_tons > max_avg_tons:
            max_avg_tons = latest_produce_tons
            is_best = True

        # 如果到了保存间隔或者当前奖励是最高的，则保存模型
        ...
        for step in range(args.num_steps): # 对于每轮迭代中应该满足的步数
            ...
            # 执行动作，envs.step此处为关键；前面已经创建了环境启动了实验，此处作为主进程将决策返回子进程，实验继续
            # 主进程进行决策，决策的结果返回实验
            next_obs_np, reward, terminations, truncations, infos = envs.step(action.cpu().numpy())           
            # 1. 将50个动作分发给50个子进程
            # 2. 每个子进程执行矿山仿真一步 
            # 3. 主进程阻塞等待 所有50个子进程完成  
            # 4. 收集所有环境的反馈数据  
            ...
            for idx in range(args.num_envs):
                envs.episode_rewards[idx] += reward[idx] # 在每一步都会累加当前步的奖励
            ...
            if step == args.num_steps - 1:
                ...
                if len(envs.episode_produce_tons) > 0:
                    ...
                    # avg_tons 是 当前rollout周期内所有已完成episode的平均产出吨数 。
        ...
        clipfracs = []
        for epoch in range(args.update_epochs): # 外层循环：更新轮数(默认2轮)：对同一批数据进行2轮训练
            np.random.shuffle(b_inds)  # 随机打乱批次索引：每轮开始前重新打乱数据顺序，提高训练稳定性
            for start in range(0, args.batch_size, args.minibatch_size): # 内层循环：小批次训练，将每次迭代70,000条经验分成4个小批次（每批17,500条）
                end = start + args.minibatch_size # 获取当前小批次的索引
                mb_inds = b_inds[start:end]
                
                # ppo算法，每一小批次都使用新策略网络计算状态价值等信息；结尾更新继续换新的再计算
                (
                    _,
                    newlogprob,     # 新策略的对数概率
                    entropy,        # 动作熵值
                    newvalue,       # 新的状态价值
                    sug_logprob,    # 建议动作的对数概率
                ...
                logratio = newlogprob - b_logprobs[mb_inds]  # 对数概率比
                ratio = logratio.exp()  # 概率比 π_new/π_old
                ...
                # teacher guidance (保留可用)，教师损失（模仿学习可用）
