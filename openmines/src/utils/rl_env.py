...
class MineEnv:
    ...
    def make(config_file, sug_dispatcher:str="ShortestTripDispatcher", reward_mode:str = "dense", seed_value:int=42, log: bool = False, ticks:bool=False):
        ...
        # 开始实验
        # 下面开启一个env的子进程（运行矿山环境）
        env.obs_queue = Queue()
        env.act_queue = Queue()
        # 核心构建函数为prepare_env，在此处被调用
    ...
    def reset(self, seed=42):
        ...
        # 创建子进程，运行矿山仿真
        # env.p用于运行完整的矿山仿真环境，目标函数 : prepare_env
        env.p = multiprocessing.Process(
            target=prepare_env,  # 子进程要执行的函数
            args=(env.obs_queue, env.act_queue, env.config, env.reward_mode, env.config['sim_time'], env.log, env.ticks, seed_value)
        )
        # 启动子进程
        env.p.start()
        """
