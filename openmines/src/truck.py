...
class Truck:
    ...
    def run(self, is_rl_training=False):
        ...
        # 添加边界检查以防止IndexError
        if dest_load_index < 0 or dest_load_index >= len(self.mine.load_sites):
            self.logger.error(f"错误：dest_load_index ({dest_load_index}) 超出范围，load_sites数量为 {len(self.mine.load_sites)}，使用默认值0")
            dest_load_index = 0
        ...
        while True:
            ...
            # 添加边界检查以防止IndexError
            if dest_unload_index < 0 or dest_unload_index >= len(self.mine.dump_sites):
                self.logger.error(f"错误：dest_unload_index ({dest_unload_index}) 超出范围，dump_sites数量为 {len(self.mine.dump_sites)}，使用默认值0")
                dest_unload_index = 0
