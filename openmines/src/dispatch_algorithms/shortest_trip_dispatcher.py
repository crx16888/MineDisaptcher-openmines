...
class ShortestTripDispatcher(BaseDispatcher):
    ...
    def give_init_order(self, truck: "Truck", mine: "Mine") -> int:
        ...
        # 计算最优装载点索引
        total_time = service_available_time + loading_service_time - now_time
        avl = np.argmin(total_time)
        
        # 添加边界检查
        if avl < 0 or avl >= len(mine.load_sites):
            print(f"警告：ShortestTripDispatcher计算出的索引 {avl} 超出范围，load_sites数量为 {len(mine.load_sites)}，使用默认值0")
            print(f"调试信息：service_available_time={service_available_time}")
            print(f"调试信息：loading_service_time={loading_service_time}")
            print(f"调试信息：total_time={total_time}")
            avl = 0
