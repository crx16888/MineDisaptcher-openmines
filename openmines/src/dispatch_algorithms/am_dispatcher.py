...
class AMDispatcher(BaseDispatcher):
    ...
    def load_am_model(self, model_path: str):
        ...
        # 获取动态的站点数量信息以便后续使用
        self.num_load_sites = getattr(self.agent, 'num_load_sites', 5)
        self.num_dump_sites = getattr(self.agent, 'num_dump_sites', 5)
    ...
    def _convert_action_to_site_index(self, action: int, truck: Truck, mine: Mine) -> int:
        ...
        """
        # 获取当前矿山的实际装载点和卸载点数量
        num_load_sites = len(mine.load_sites)
        num_dump_sites = len(mine.dump_sites)
        
        # 确保与模型训练时的数量一致
        if num_load_sites != self.num_load_sites or num_dump_sites != self.num_dump_sites:
            print(f"警告：当前矿山配置({num_load_sites}装载点, {num_dump_sites}卸载点)与模型训练配置({self.num_load_sites}装载点, {self.num_dump_sites}卸载点)不一致")
            # 使用模型训练时的数量以确保一致性
            num_load_sites = min(num_load_sites, self.num_load_sites)
            num_dump_sites = min(num_dump_sites, self.num_dump_sites)
        ...
        if "charging" in current_location.lower():
            # 卡车在充电站，需要去装载点
            if 1 <= action <= self.num_load_sites:
                site_index = action - 1  # 转换为装载点索引 0 到 num_load_sites-1
                # 添加边界检查，确保不超出实际可用的装载点数量
                if site_index >= len(mine.load_sites):
                    print(f"警告：AM装载点索引 {site_index} 超出实际范围，实际装载点数量为 {len(mine.load_sites)}，使用默认值0")
                    return 0
                return site_index
            else:
                # 如果动作不合法，默认去第一个装载点
                print(f"警告：充电站处收到无效动作 {action}，期望范围 1-{self.num_load_sites}，使用默认值0")
                return 0
                
        elif "load" in current_location.lower():
            # 卡车在装载点，需要去卸载点
            dump_action_start = self.num_load_sites + 1
            dump_action_end = self.num_load_sites + self.num_dump_sites
            if dump_action_start <= action <= dump_action_end:
                site_index = action - dump_action_start  # 转换为卸载点索引
                # 添加边界检查，确保不超出实际可用的卸载点数量
                if site_index >= len(mine.dump_sites):
                    print(f"警告：AM卸载点索引 {site_index} 超出实际范围，实际卸载点数量为 {len(mine.dump_sites)}，使用默认值0")
                    return 0
                return site_index
            else:
                # 如果动作不合法，默认去第一个卸载点
                print(f"警告：装载点处收到无效动作 {action}，期望范围 {dump_action_start}-{dump_action_end}，使用默认值0")
                return 0
                
        elif "dump" in current_location.lower():
            # 卡车在卸载点，需要返回装载点或充电站
            if 1 <= action <= self.num_load_sites:
                site_index = action - 1  # 转换为装载点索引
                # 添加边界检查，确保不超出实际可用的装载点数量
                if site_index >= len(mine.load_sites):
                    print(f"警告：AM装载点索引 {site_index} 超出实际范围，实际装载点数量为 {len(mine.load_sites)}，使用默认值0")
                    return 0
                ...
                # 默认去第一个装载点
                print(f"警告：卸载点处收到无效动作 {action}，期望范围 1-{self.num_load_sites}，使用默认值0")
                return 0
        else:
            # 未知状态，默认动作
            print(f"警告：未知位置 {current_location}，使用默认值0")
            return 0
