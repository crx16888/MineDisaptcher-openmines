...
class PPODispatcher(BaseDispatcher):
    def __init__(self, model_path: Optional[str] = None):
        ...
        if model_path is not None:
            ...
            print(f"自动找到最佳模型: {self.model_path}")
    ...
    def _find_latest_best_model(self):
        ...
        """
        # 检查默认的模型目录
        default_model_path = os.path.join(os.path.dirname(__file__), "checkpoints", "mine", "Mine-v1__ppo_single_net__s1__lr2.28e-03__e1.43e-02__g0.997__c0.20__l0.993__ep4__gr0.36__hs256__ns1400__ne50__mb4__rmreward_norm__t1752899453/best_model_step9170000_tons9521.9_reward0.00.pt")
        
        if os.path.exists(default_model_path):
            return default_model_path
        
        # 如果默认模型不存在，尝试在checkpoints目录中查找最新的模型
        checkpoints_dir = os.path.join(os.path.dirname(__file__), "checkpoints", "mine")
        if os.path.exists(checkpoints_dir):
            ...
            if model_files:
                # 按文件名排序（通常包含步数或时间戳），选择最新的
                model_files.sort(reverse=True)
                return model_files[0]
        
        # 如果都找不到，返回默认路径（会触发FileNotFoundError）
        return default_model_path
    ...
    def load_rl_model(self, model_path: str):
        ...
        self.agent = Agent(envs=-1, args=self.args, 
                         norm_path="/home/chengrongxian/git/openmines0/datasets/dispatch_data_20250719_122119/normalization_params.json") # 使用训练时的正则化参数
...
if __name__ == "__main__":
    ...
    # 示例：使用指定模型
    # dispatcher = PPODispatcher(model_path="/path/to/your/model.pt")
